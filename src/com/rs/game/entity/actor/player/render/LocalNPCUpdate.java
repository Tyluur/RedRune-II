package com.rs.game.entity.actor.player.render;
	...
	private void processInScreenNPCs(OutputStream stream,
		...
		for (Iterator<NPC> it = localNPCs.iterator(); it.hasNext();) {
			...
			boolean needUpdate = (updateBlockData.getOffset()
					+ stream.getOffset() < (NetworkConstants.PACKET_SIZE_LIMIT - 500))
					&& n.needMasksUpdate();
		...
		for (int regionId : player.getMapRegionsIds()) {
			...
			for (int npcIndex : indexes) {
				if (localNPCs.size() == GameConstants.LOCAL_NPCS_LIMIT
						|| (updateBlockData.getOffset() + stream.getOffset() > (NetworkConstants.PACKET_SIZE_LIMIT - 500)))
					break;
