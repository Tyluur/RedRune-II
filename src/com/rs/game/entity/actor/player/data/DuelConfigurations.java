...
public class DuelConfigurations {
	 ...
	 */
	public WorldTile DUELARENA_LOBBY = new WorldTile(3365, 3275, 0);
	...
	public void endDuel(Player player, boolean declined, boolean logout) {
		...
		if (!declined) {
			if (logout)
				...
				player.setNextWorldTile(DUELARENA_LOBBY);
		} else {
	...
	public void sendInventory(Player player) {
		...
		player.getPackets().sendUnlockIComponentOptionSlots(631, 103, 0, 27, 0,
				1, 2, 3, 4, 5);
		player.getPackets().sendInterSetItemsOptionsScript(631, 0, 134, 4, 7,
	...
	private void declineDuel(Player player) {
		...
		other.getControlerManager().startControler("DuelControler");
	}
	...
	public void handleAccept(final Player player, boolean ifFriendly) {
		if (canAccept(player)) {
			...
			other.getPackets().sendIComponentText(ifFriendly ? 637 : 631,
	...
	public void handleSecondScreenAccept(final Player player, boolean ifFriendly) {
		...
		player.getTemporaryAttributtes().put("accepted", true);
		other.getPackets().sendIComponentText(ifFriendly ? 639 : 626,
				ifFriendly ? 23 : 35, "Other player has accepted.");
		player.getPackets().sendIComponentText(ifFriendly ? 639 : 626,
	...
	private void teleportPlayer(Player player, Player other) {
		...
		player.getControlerManager().startControler("Duelarena");
		other.getControlerManager().startControler("Duelarena");
	}
	...
	public void refresh(int... slots) {
		...
		player.getPackets().sendUpdateItems(93,
				player.getInventory().getItems(), slots);
		target.getPackets().sendUpdateItems(93,
	...
	public void addSpoils(Player player) {
		...
		if (player == this.player)
			other.getPackets().sendItems(136, false, stake);

		if (player == target)
			other.getPackets().sendItems(136, false, stake2);

		other.getPackets().sendUnlockIComponentOptionSlots(634, 28, 0, 35, 0,
				1, 2, 3, 4, 5);
		other.getPackets().sendInterSetItemsOptionsScript(634, 28, 136, 4, 7,
				"", "", "", "", "");
		other.getPackets().sendIComponentText(634, 23,
				Misc.formatPlayerNameForDisplay(player.getUsername()));
		other.getPackets().sendIComponentText(634, 22,
