...
public final class Nex extends NPC {
	...
	public void sendDeath(Actor source) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				ArrayList<Actor> possibleTargets = getPossibleTargets();
				if (possibleTargets != null) {
					for (Actor actor : possibleTargets) {
						if (actor == null || actor.isDead()
								|| actor.hasFinished()
								|| !actor.withinDistance(target, 10))
							continue;
						...
						World.sendProjectile(target, new WorldTile(getX(),
								getY() - 2, getPlane()), 2260, 41, 0, 41, 35,
								30, 0);
						actor.applyHit(new Hit(target, Misc.getRandom(600),
	...
	public ArrayList<Actor> calculatePossibleTargets(WorldTile current,
			WorldTile position, boolean northSouth) {
		ArrayList<Actor> list = new ArrayList<Actor>();
		for (Actor e : getPossibleTargets()) {
	...
	public void sendVirusAttack(ArrayList<Actor> hitedActors,
			ArrayList<Actor> possibleTargets, Actor infected) {
		for (Actor t : possibleTargets) {
			if (hitedActors.contains(t))
				continue;
			if (Misc.getDistance(t.getX(), t.getY(), infected.getX(),
				...
				t.applyHit(new Hit(this, Misc.getRandom(100),
						HitLook.REGULAR_DAMAGE));
				hitedActors.add(t);
				sendVirusAttack(hitedActors, possibleTargets, infected);
			}
		}
		playSound(3296, 2);
	}
