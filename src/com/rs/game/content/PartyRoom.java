...
public class PartyRoom {
	...
	private static WorldTile[] dancingTiles = { new WorldTile(3049, 3378, 0), new WorldTile(3048, 3378, 0), new WorldTile(3047, 3378, 0), new WorldTile(3046, 3378, 0), new WorldTile(3045, 3378, 0), new WorldTile(3044, 3378, 0), new WorldTile(3043, 3378, 0), new WorldTile(3042, 3378, 0) };
		...
		if (worldwide) {
			for (NPC npc : World.getNPCs()) {
				...
				if (npc.getName().toLowerCase().contains("banker")) {
					npc.setNextForceTalk(new ForceTalk(Misc.getFormattedNumber(getDropValue() / getDropValue() > 1000000000 ? 1000000000 : 1000000) + " " + (getDropValue() > 1000000000 ? "B" : "M") + " Drop party in falador party room in " + getTimeLeft() + " " + (getTimeLeft() > 1 ? "s" : "") + "."));
				}
			}
		}
	}
