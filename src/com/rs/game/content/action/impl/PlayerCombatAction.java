...
public class PlayerCombatAction extends Action {
	...
	private boolean checkAll(Player player) {
		...
		if (target instanceof Player) {
			...
			if (player.getTemporaryAttributtes().get("InstantSpecial") != null) {
				int specialId = (int) player.getTemporaryAttributtes().get("InstantSpecial");
			...
			if (p2.getTemporaryAttributtes().get("InstantSpecial") != null) {
				int specialId = (int) p2.getTemporaryAttributtes().get("InstantSpecial");
			...
			NPC n = (NPC) target;
			if (!Slayer.checkRequirement(player, SlayerMonsters.forId(n.getId()))) {
				return false;
			}
			if (n.isCantInteract()) {
				return false;
		...
		int isRanging = isRanging(player);
		if (player.getX() == target.getX() && player.getY() == target.getY() && target.getSize() == 1 && !target.hasWalkSteps()) {
		...
		} else if (isRanging == 0 && target.getSize() == 1 && player.getCombatDefinitions().getSpellId() <= 0 && Math.abs(player.getX() - target.getX()) == 1 && Math.abs(player.getY() - target.getY()) == 1 && !target.hasWalkSteps()) {
	...
	public static final int getMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean usingSpec, double specMultiplier) {
		if (!ranging) {
			int strengthLvl = player.getSkills().getLevel(PlayerSkills.STRENGTH);
			double xpStyle = CombatDefinitions.getXpStyle(weaponId, attackStyle);
			double styleBonus = xpStyle == PlayerSkills.STRENGTH ? 3 : xpStyle == CombatDefinitions.SHARED ? 1 : 0;
			double otherBonus = 1;
			...
			double effectiveStrength = 8 + strengthLvl * player.getPrayer().getStrengthMultiplier() + styleBonus;
			...
			double rangedLvl = player.getSkills().getLevel(PlayerSkills.RANGE);
			double styleBonus = attackStyle == 0 ? 3 : attackStyle == 1 ? 0 : 1;
			double otherBonus = 1;
			double effectiveStrenght = (rangedLvl * player.getPrayer().getRangeMultiplier() * otherBonus) + styleBonus;
			if (fullVoidEquipped(player, 11664, 11675)) {
				effectiveStrenght += (player.getSkills().getLevelForXp(PlayerSkills.RANGE) / 5) + 1.6;
			}
	...
	public static final boolean usingGoliathGloves(Player player) {
		String name = player.getEquipment().getItem(EquipmentConstants.SLOT_SHIELD) != null ? player.getEquipment().getItem(EquipmentConstants.SLOT_SHIELD).getDefinitions().getName().toLowerCase() : "";
		if (player.getEquipment().getItem((EquipmentConstants.SLOT_HANDS)) != null) {
			if (player.getEquipment().getItem(EquipmentConstants.SLOT_HANDS).getDefinitions().getName().toLowerCase().contains("goliath") && player.getEquipment().getWeaponId() == -1) {
	...
	private void addAttackedByDelay(Actor player) {
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			switch (spellId) {
				...
				case 28: // water strike
					...
					mage_hit_gfx = 2708;
					base_mage_xp = 7.5;
					delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 40)));
				...
				case 55:// snare
					...
					if (snareHit.getDamage() > 0 && target.getFrozenBlockedDelay() < Misc.currentTimeMillis()) {
				...
				case 81:// entangle
					...
					if (entangleHit.getDamage() > 0 && target.getFrozenBlockedDelay() < Misc.currentTimeMillis()) {
				...
				case 30: // earth strike
					...
					mage_hit_gfx = 2723;
					base_mage_xp = 9.5;
					delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 60)));
				...
				case 42: // earth bolt
					...
					mage_hit_gfx = 2724;
					base_mage_xp = 19.5;
					delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 110)));
				...
				case 80: // fire wave
					...
					return 5;
				case 86: // teleblock
					if (target instanceof Player && ((Player) target).getTeleBlockDelay() <= Misc.currentTimeMillis()) {
				...
				case 99: // Storm of armadyl //Sonic and Tyler dumped
					...
					mage_hit_gfx = 1019;
					base_mage_xp = 70;
					int boost = (player.getSkills().getLevelForXp(PlayerSkills.MAGIC) - 77) * 5;
			...
			switch (spellId) {
				...
				case 21:// Ice Blitz
					...
					freeze_time = 15000;
					magic_sound = 169;
					delayMagicHit(4, getMagicHit(player, getRandomMagicMaxHit(player, 260)));
					return 4;
				case 23: // ice barrage
	...
	public int getRandomMagicMaxHit(Player player, int baseDamage) {
		int hit = Misc.getRandom(getMagicMaxHit(player, baseDamage));
		if (hit != 0) {
			if (target instanceof NPC) {
				NPC n = (NPC) target;
	...
	public int getMagicMaxHit(Player player, int baseDamage) {
		...
		double att = ((player.getSkills().getLevel(PlayerSkills.MAGIC) + 8) * player.getPrayer().getMageMultiplier());
		int style = CombatDefinitions.getXpStyle(player.getEquipment().getWeaponId(), player.getCombatDefinitions().getAttackStyle());
		att += style == PlayerSkills.ATTACK ? 3 : style == CombatDefinitions.SHARED ? 1 : 0;
		...
		if (target instanceof Player) {
			Player p2 = (Player) target;
			int p2style = CombatDefinitions.getXpStyle(p2.getEquipment().getWeaponId(), p2.getCombatDefinitions().getAttackStyle());
			def = ((p2.getSkills().getLevel(PlayerSkills.MAGIC) + 8) + p2.getPrayer().getDefenceMultiplier() + p2style == PlayerSkills.DEFENCE ? 3 : p2style == CombatDefinitions.SHARED ? 1 : 0);
			defBonus = p2.getCombatDefinitions().getBonuses()[CombatDefinitions.MAGIC_DEF];
			effectiveMagicDefence = Math.floor((double) p2.getSkills().getLevel(PlayerSkills.MAGIC));
		} else {
			NPC n = (NPC) target;
		...
		double boost = 1 + ((player.getSkills().getLevel(PlayerSkills.MAGIC) - player.getSkills().getLevelForXp(PlayerSkills.MAGIC)) * 0.03);
		if (boost > 1) {
			max_hit *= boost;
		}
		return max_hit;
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			switch (weaponId) {
				...
				case 14684: // zanik cbow
					...
					delayHit(2, weaponId, attackStyle, getRangeHit(player, getRandomMaxHit(player, weaponId, attackStyle, true, true, 1.0, true) + 30 + Misc.getRandom(120)));
					dropAmmo(player);
					break;
				case 13954:// morrigan javelin
				case 12955:
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe") || weaponName.contains("knife") || weaponName.contains("dart") || weaponName.contains("javelin") || weaponName.contains("thrownaxe")) {
					...
					if (Misc.getRandom(player.getSkills().getLevel(PlayerSkills.FIREMAKING)) == 0) {
						// explode
						player.setNextGraphics(new Graphics(2140));
						player.getEquipment().getItems().set(3, null);
						player.getEquipment().refresh(3);
						player.getAppearance().generateAppearanceData();
						player.applyHit(new Hit(player, Misc.getRandom(150) + 10, HitLook.REGULAR_DAMAGE));
					...
					base_mage_xp = 19;
					mage_hit_gfx = 2036;
					delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 585)));
					World.sendProjectile(player, target, 2035, 18, 18, 50, 50, 0, 0);
					return 5;
				} else { // bow/default
					final int ammoId = player.getEquipment().getAmmoId();
					player.setNextGraphics(new Graphics(getArrowThrowGfxId(weaponId, ammoId), 0, 100));
					World.sendProjectile(player, target, getArrowProjectileGfxId(weaponId, ammoId), 41, 16, 20, 35, 16, 0);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, getRandomMaxHit(player, weaponId, attackStyle, true)));
					if (weaponId == 11235 || weaponId == 15701 || weaponId == 15702 || weaponId == 15703 || weaponId == 15704) { // dbows
						World.sendProjectile(player, target, getArrowProjectileGfxId(weaponId, ammoId), 41, 35, 26, 35, 21, 0);
	...
	private int getRangeHitDelay(Player player) {
		return Misc.getDistance(player.getX(), player.getY(), target.getX(), target.getY()) >= 5 ? 2 : 1;
	}
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			switch (weaponId) {
				...
				case 11730: // sara sword
					...
					delayNormalHit(weaponId, attackStyle, getMeleeHit(player, 50 + Misc.getRandom(100)), getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.1, true)));
					soundId = 3853;
					break;
				case 1249:// d spear
				case 1263:
				...
				case 13776:
					...
					target.setNextFaceActor(player);
					WorldTasksManager.schedule(new WorldTask() {
						@Override
						public void run() {
							target.setNextFaceActor(null);
							player.setNextFaceActor(null);
						}
					});
					if (target instanceof Player) {
						...
						other.setEquipDisabled(true);
						WorldTasksManager.schedule(new WorldTask() {
							@Override
							public void run() {
								other.setEquipDisabled(false);
								other.resetLockDelay();
							}
						}, 5);
					} else {
						NPC n = (NPC) target;
				...
				case 11696: // bgs
					...
					if (target instanceof Player) {
						...
						int amountLeft;
						if ((amountLeft = targetPlayer.getSkills().drainLevel(PlayerSkills.DEFENCE, damage / 10)) > 0) {
							if ((amountLeft = targetPlayer.getSkills().drainLevel(PlayerSkills.STRENGTH, amountLeft)) > 0) {
								if ((amountLeft = targetPlayer.getSkills().drainLevel(PlayerSkills.PRAYER, amountLeft)) > 0) {
									if ((amountLeft = targetPlayer.getSkills().drainLevel(PlayerSkills.ATTACK, amountLeft)) > 0) {
										if ((amountLeft = targetPlayer.getSkills().drainLevel(PlayerSkills.MAGIC, amountLeft)) > 0) {
											if (targetPlayer.getSkills().drainLevel(PlayerSkills.RANGE, amountLeft) > 0) {
												break;
											}
										}
									}
				...
				case 11694: // ags
					...
					delayNormalHit(weaponId, attackStyle, getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.50, true)));
					break;
				case 13899: // vls
				case 13901:
				...
				case 14484: // d claws
					...
					if (hit1 == 0 && hit2 == 0) {
						...
						if (hit3 == 0) {
							...
							if (hit4 == 0) {
								delayNormalHit(weaponId, attackStyle, getMeleeHit(player, hit1), getMeleeHit(player, hit2));
								delayHit(1, weaponId, attackStyle, getMeleeHit(player, hit3), getMeleeHit(player, 1));
							} else {
								delayNormalHit(weaponId, attackStyle, getMeleeHit(player, hit1), getMeleeHit(player, hit2));
								delayHit(1, weaponId, attackStyle, getMeleeHit(player, hit3), getMeleeHit(player, (int) (hit4 * 1.5)));
							}
						} else {
							delayNormalHit(weaponId, attackStyle, getMeleeHit(player, hit1), getMeleeHit(player, hit2));
						...
						delayNormalHit(weaponId, attackStyle, getMeleeHit(player, hit1), getMeleeHit(player, hit1 == 0 ? hit2 : hit2 / 2));
						delayHit(1, weaponId, attackStyle, getMeleeHit(player, hit1 == 0 ? hit2 / 2 : hit2 / 4), getMeleeHit(player, hit2 / 4));
					}
					break;
				case 10887: // anchor
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean defenceAffects, double specMultiplier, boolean usingSpec) {
		...
		if (defenceAffects) {
			...
			if (target instanceof Player) {
				Player p2 = (Player) target;
				def = (double) p2.getSkills().getLevel(PlayerSkills.DEFENCE) + (2 * p2.getCombatDefinitions().getBonuses()[ranging ? 9 : CombatDefinitions.getMeleeDefenceBonus(CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle))]);
	...
	private void delayHit(int delay, final int weaponId, final int attackStyle, final Hit... hits) {
		...
		final Actor target = this.target;
		for (Hit hit : hits) {
			...
			if (hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MELEE_DAMAGE) {
				...
				if (combatXp > 0) {
					...
					if (hit.getLook() == HitLook.RANGE_DAMAGE) {
						if (attackStyle == 2) {
							player.getSkills().addXp(PlayerSkills.RANGE, combatXp / 2);
							player.getSkills().addXp(PlayerSkills.DEFENCE, combatXp / 2);
						} else {
							player.getSkills().addXp(PlayerSkills.RANGE, combatXp);
						}
						...
						if (xpStyle != CombatDefinitions.SHARED) {
							...
							player.getSkills().addXp(PlayerSkills.ATTACK, combatXp / 3);
							player.getSkills().addXp(PlayerSkills.STRENGTH, combatXp / 3);
							player.getSkills().addXp(PlayerSkills.DEFENCE, combatXp / 3);
						}
					}
					double hpXp = (damage / 10) * 1.2342;
					if (hpXp > 0) {
						player.getSkills().addXp(PlayerSkills.HITPOINTS, hpXp);
					}
				}
				...
				if (mage_hit_gfx != 0 && damage > 0) {
					if (freeze_time > 0) {
						...
						if (freeze_time > 0) {
							target.addFrozenBlockedDelay(freeze_time + (4 * 1000));// foure seconds :p
						}
					}
				}
				...
				if (combatXp > 0) {
					...
					if (player.getCombatDefinitions().isDefensiveCasting()) {
						...
						if (defenceXp > 0) {
							combatXp -= defenceXp;
							player.getSkills().addXp(PlayerSkills.DEFENCE, defenceXp / 7.5);
						}
					}
					player.getSkills().addXp(PlayerSkills.MAGIC, combatXp);
				...
				for (Hit hit : hits) {
					...
					if (hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MELEE_DAMAGE) {
						...
						if (combatXp > 0) {
							if (hit.getLook() == HitLook.RANGE_DAMAGE) {
								if (weaponId != -1) {
									...
									if (name.contains("(p++)")) {
										if (Misc.getRandom(8) == 0) {
											target.getPoisonManager().makePoisoned(48);
										}
									} else if (name.contains("(p+)")) {
										if (Misc.getRandom(8) == 0) {
											target.getPoisonManager().makePoisoned(38);
										}
									} else if (name.contains("(p)")) {
										if (Misc.getRandom(8) == 0) {
											target.getPoisonManager().makePoisoned(28);
										}
									}
								}
							} else {
								if (weaponId != -1) {
									...
									if (name.contains("(p++)")) {
										if (Misc.getRandom(8) == 0) {
											target.getPoisonManager().makePoisoned(68);
										}
									} else if (name.contains("(p+)")) {
										if (Misc.getRandom(8) == 0) {
											target.getPoisonManager().makePoisoned(58);
										}
									} else if (name.contains("(p)")) {
										if (Misc.getRandom(8) == 0) {
											target.getPoisonManager().makePoisoned(48);
									...
									if (target instanceof Player) {
										if (((Player) target).getPolDelay() >= Misc.currentTimeMillis()) {
						...
						if (hit.getDamage() == 0) {
							...
							if (mage_hit_gfx != 0) {
								target.setNextGraphics(new Graphics(mage_hit_gfx, 0, mage_hit_gfx == 369 || mage_hit_gfx == 1843 ? 0 : 100));
								if (blood_spell) {
					...
					if (max_poison_hit > 0 && Misc.getRandom(10) == 0) {
						if (!target.getPoisonManager().isPoisoned()) {
							target.getPoisonManager().makePoisoned(max_poison_hit);
						}
						max_poison_hit = 0;
					}
					if (target instanceof Player) {
						...
						if (p2.getCombatDefinitions().isAutoRelatie() && !p2.getActionManager().hasSkillWorking() && !p2.hasWalkSteps()) {
							p2.getActionManager().setAction(new PlayerCombatAction(player));
						}
					} else {
						NPC n = (NPC) target;
	...
	private int getMeleeCombatDelay(Player player, int weaponId) {
		if (weaponId != -1) {
			...
			// Interval 3.0
			if (weaponName.contains("spear") || weaponName.contains("longsword") || weaponName.contains("light") || weaponName.contains("hatchet") || weaponName.contains("pickaxe") || weaponName.contains("mace") || weaponName.contains("hasta") || weaponName.contains("warspear") || weaponName.contains("flail") || weaponName.contains("hammers")) {
				return 4;
			}
			// Interval 3.6
			if (weaponName.contains("godsword") || weaponName.contains("warhammer") || weaponName.contains("battleaxe") || weaponName.contains("maul")) {
				return 5;
			}
			// Interval 4.2
			if (weaponName.contains("greataxe") || weaponName.contains("halberd") || weaponName.contains("2h sword") || weaponName.contains("two handed sword")) {
				return 6;
			}
		}
		switch (weaponId) {
