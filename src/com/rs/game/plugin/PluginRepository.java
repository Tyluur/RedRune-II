...
public class PluginRepository {
	 ...
	 */
	private static final Map<Integer, Map<ClickOption, ObjectPlugin>> OBJECT_PLUGINS = new HashMap<>();
	...
	public static void registerOptionablePlugin(Plugin plugin, int key, ClickOption... options) {
		if (plugin instanceof NPCPlugin) {
			...
			Map<ClickOption, ObjectPlugin> pluginMap = new HashMap<>();
			for (ClickOption option : options) {
	 ...
	 * 		The option clicked
	 */
	public static boolean handleObject(Player player, WorldObject object, ClickOption option) {
		Map<ClickOption, ObjectPlugin> pluginMap = OBJECT_PLUGINS.get(object.getId());
		if (pluginMap == null) {
			return false;
		}
		...
		plugin.handle(player, object, option);
		return true;
	}
