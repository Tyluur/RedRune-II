...
public class PlayerPrayer implements Serializable {
    ...
    private boolean checkPrayer() {
        if (prayerpoints <= 0) {
            ...
            player.getPackets().sendMessage("Please recharge your prayer at the Lumbridge Church.");
            return false;
        }
        return true;
    }
    ...
    private boolean usePrayer(int prayerId) {
        ...
        if (player.getSkills().getLevelForXp(5) < prayerLvls[this.getPrayerBook()][prayerId]) {
            player.getPackets().sendMessage("You need a prayer level of at least " + prayerLvls[getPrayerBook()][prayerId] + " to use this prayer.");
            return false;
        }
        if (getPrayerBook() == 0 && prayerId == 25 || prayerId == 27) {
            if (player.getSkills().getLevelForXp(SkillConstants.DEFENCE) < 60) {
                player.getPackets().sendMessage("You need a defence level of at least " + 60 + " to use this prayer.");
                return false;
            }
        } else if (getPrayerBook() == 1) {
            if (player.getSkills().getLevelForXp(SkillConstants.DEFENCE) < 30) {
                player.getPackets().sendMessage("You need a defence level of at least " + 30 + " to use this prayer.");
                return false;
            }
        }
        if (player.getAttributes().getPrayerDelay() >= Misc.currentTimeMillis()) {
            player.getPackets().sendMessage("You are currently injured and cannot use protection prayers!");
    ...
    public void closePrayers(int prayerId) {
        if (ancientcurses) {
            if (prayerId == 1) {
                if (leechBonuses[0] > 0) {
                    player.getPackets().sendMessage("Your Attack is now unaffected by sap and leech curses.", true);
                }
                adjustStat(0, 0);
                adjustStat(1, 0);
                adjustStat(2, 0);
                ...
                if (leechBonuses[1] > 0) {
                    player.getPackets().sendMessage("Your Range is now unaffected by sap and leech curses.", true);
                }
                adjustStat(2, 0);
                adjustStat(4, 0);
                leechBonuses[1] = 0;
                ...
                if (leechBonuses[2] > 0) {
                    player.getPackets().sendMessage("Your Magic is now unaffected by sap and leech curses.", true);
                }
                adjustStat(2, 0);
                adjustStat(5, 0);
                leechBonuses[2] = 0;
                ...
                if (leechBonuses[4] > 0) {
                    player.getPackets().sendMessage("Your Ranged is now unaffected by sap and leech curses.", true);
                }
                adjustStat(4, 0);
                leechBonuses[4] = 0;
            } else if (prayerId == 12) {
                ...
                if (leechBonuses[6] > 0) {
                    player.getPackets().sendMessage("Your Defence is now unaffected by sap and leech curses.", true);
                }
                adjustStat(2, 0);
                leechBonuses[6] = 0;
            } else if (prayerId == 14) {
                if (leechBonuses[7] > 0) {
                    player.getPackets().sendMessage("Your Strength is now unaffected by sap and leech curses.", true);
                }
                adjustStat(1, 0);
                leechBonuses[7] = 0;
            } else if (prayerId == 19) {
     ...
     *
     * @param source The source of the kill
     */
    ...
    public void handleCurseBoosts(Actor target, Hit hit) {
        if (player.getPrayer().usingPrayer(1, 18)) {
            sendSoulSplit(hit, target);
        }
        if (player.getPrayer().usingPrayer(0, 24)) { // smite
            ...
            if (!player.getPrayer().isBoostedLeech()) {
                if (hit.getSplat() == HitSplat.MELEE_DAMAGE) {
                    ...
                    } else if (player.getPrayer().usingPrayer(1, 1)) { // sap att
                        if (Misc.getRandom(4) == 0) {
                            if (player.getPrayer().reachedMax(0)) {
                                ...
                                player.getPackets().sendMessage("Your curse drains Attack from the enemy, boosting your Attack.", true);
                            }
                            player.setNextAnimation(new Animation(12569));
                            player.setNextGraphics(new Graphics(2214));
                            player.getPrayer().setBoostedLeech(true);
                            RegionManager.sendProjectile(player, target, 2215, 35, 35, 20, 5, 0, 0);
                            WorldTasksManager.schedule(new WorldTask() {
                                @Override
                                public void run() {
                        ...
                        if (player.getPrayer().usingPrayer(1, 10)) {
                            if (Misc.getRandom(7) == 0) {
                                if (player.getPrayer().reachedMax(3)) {
                                    player.getPackets().sendMessage("Your opponent has been weakened so much that your leech curse has no effect.", true);
                                } else {
                                ...
                                player.getPrayer().setBoostedLeech(true);
                                RegionManager.sendProjectile(player, target, 2231, 35, 35, 20, 5, 0, 0);
                                WorldTasksManager.schedule(new WorldTask() {
                                    @Override
                                    public void run() {
                        ...
                        if (player.getPrayer().usingPrayer(1, 14)) {
                            if (Misc.getRandom(7) == 0) {
                                if (player.getPrayer().reachedMax(7)) {
                                    ...
                                    player.getPackets().sendMessage("Your curse drains Strength from the enemy, boosting your Strength.", true);
                                }
                                player.setNextAnimation(new Animation(12575));
                                player.getPrayer().setBoostedLeech(true);
                                RegionManager.sendProjectile(player, target, 2248, 35, 35, 20, 5, 0, 0);
                                WorldTasksManager.schedule(new WorldTask() {
                                    @Override
                                    public void run() {
                    ...
                    if (player.getPrayer().usingPrayer(1, 2)) { // sap range
                        if (Misc.getRandom(4) == 0) {
                            if (player.getPrayer().reachedMax(1)) {
                                ...
                                player.getPackets().sendMessage("Your curse drains Range from the enemy, boosting your Range.", true);
                            }
                            player.setNextAnimation(new Animation(12569));
                            player.setNextGraphics(new Graphics(2217));
                            player.getPrayer().setBoostedLeech(true);
                            RegionManager.sendProjectile(player, target, 2218, 35, 35, 20, 5, 0, 0);
                            WorldTasksManager.schedule(new WorldTask() {
                                @Override
                                public void run() {
                        ...
                        if (Misc.getRandom(7) == 0) {
                            ...
                            player.setNextAnimation(new Animation(12575));
                            player.getPrayer().setBoostedLeech(true);
                            RegionManager.sendProjectile(player, target, 2236, 35, 35, 20, 5, 0, 0);
                            WorldTasksManager.schedule(new WorldTask() {
                                @Override
                                public void run() {
                ...
                if (hit.getSplat() == HitSplat.MAGIC_DAMAGE) {
                    if (player.getPrayer().usingPrayer(1, 3)) { // sap mage
                        if (Misc.getRandom(4) == 0) {
                            if (player.getPrayer().reachedMax(2)) {
                                player.getPackets().sendMessage("Your opponent has been weakened so much that your sap curse has no effect.", true);
                            } else {
                                player.getPrayer().increaseLeechBonus(2);
                                player.getPackets().sendMessage("Your curse drains Magic from the enemy, boosting your Magic.", true);
                            }
                            player.setNextAnimation(new Animation(12569));
                            player.setNextGraphics(new Graphics(2220));
                            player.getPrayer().setBoostedLeech(true);
                            RegionManager.sendProjectile(player, target, 2221, 35, 35, 20, 5, 0, 0);
                            WorldTasksManager.schedule(new WorldTask() {
                                @Override
                                public void run() {
                        ...
                        if (Misc.getRandom(7) == 0) {
                            ...
                            player.setNextAnimation(new Animation(12575));
                            player.getPrayer().setBoostedLeech(true);
                            RegionManager.sendProjectile(player, target, 2240, 35, 35, 20, 5, 0, 0);
                            WorldTasksManager.schedule(new WorldTask() {
                                @Override
                                public void run() {
                ...
                if (player.getPrayer().usingPrayer(1, 13)) { // leech defence
                    if (Misc.getRandom(10) == 0) {
                        if (player.getPrayer().reachedMax(6)) {
                            ...
                            player.getPackets().sendMessage("Your curse drains Defence from the enemy, boosting your Defence.", true);
                        }
                        player.setNextAnimation(new Animation(12575));
                        player.getPrayer().setBoostedLeech(true);
                        RegionManager.sendProjectile(player, target, 2244, 35, 35, 20, 5, 0, 0);
                        WorldTasksManager.schedule(new WorldTask() {
                            @Override
                            public void run() {
                ...
                if (player.getPrayer().usingPrayer(1, 15)) {
                    if (Misc.getRandom(10) == 0) {
                        if (player.getAttributes().getRunEnergy() <= 0) {
                            player.getPackets().sendMessage("Your opponent has been weakened so much that your leech curse has no effect.", true);
                        } else {
                            player.getAttributes().setRunEnergy(player.getAttributes().getRunEnergy() > 90 ? 100 : player.getAttributes().getRunEnergy() + 10);
                            if (target instanceof NPC) {
                                Player p2 = (Player) target;
                                p2.getAttributes().setRunEnergy(p2.getAttributes().getRunEnergy() > 10 ? p2.getAttributes().getRunEnergy() - 10 : 0);
                            }
                        }
                        player.setNextAnimation(new Animation(12575));
                        player.getPrayer().setBoostedLeech(true);
                        RegionManager.sendProjectile(player, target, 2256, 35, 35, 20, 5, 0, 0);
                        WorldTasksManager.schedule(new WorldTask() {
                            @Override
                            public void run() {
                ...
                if (player.getPrayer().usingPrayer(1, 16)) {
                    if (Misc.getRandom(10) == 0) {
                        ...
                        player.setNextAnimation(new Animation(12575));
                        player.getPrayer().setBoostedLeech(true);
                        RegionManager.sendProjectile(player, target, 2252, 35, 35, 20, 5, 0, 0);
                        WorldTasksManager.schedule(new WorldTask() {
                            @Override
                            public void run() {
                ...
                if (player.getPrayer().usingPrayer(1, 4)) { // sap spec
                    if (Misc.getRandom(10) == 0) {
                        player.setNextAnimation(new Animation(12569));
                        ...
                        RegionManager.sendProjectile(player, target, 2224, 35, 35, 20, 5, 0, 0);
                        WorldTasksManager.schedule(new WorldTask() {
                            @Override
                            public void run() {
    ...
    public void sendSoulSplit(final Hit hit, final Actor target) {
        if (hit.getDamage() > 0) {
            RegionManager.sendProjectile(player, target, 2263, 11, 11, 20, 5, 0, 0);
        }
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (hit.getDamage() > 0) {
                    RegionManager.sendProjectile(target, player, 2263, 11, 11, 20, 5, 0, 0);
                }
            }
        }, 1);
    }
