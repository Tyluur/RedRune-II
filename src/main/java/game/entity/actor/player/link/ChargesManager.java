		...
		for (int slot = 0; slot < items.length; slot++) {
			...
			if (player.getAttackedByDelay() > Misc.currentTimeMillis()) {
				...
				if (newId != -1) {
					...
					player.getPackets().sendMessage("Your " + item.getDefinitions().getName() + " degraded.");
				}
			}
	...
	private void degrade(int itemId, int defaultCharges, int slot) {
		...
		if (c == null) {
			...
			if (c == 0) {
				...
				if (newId == -1) {
					player.getPackets().sendMessage("Your " + ItemDefinitions.getItemDefinitions(itemId).getName() + " became into dust.");
				} else {
					player.getPackets().sendMessage("Your " + ItemDefinitions.getItemDefinitions(itemId).getName() + " degraded.");
				}
				player.getEquipment().refresh(slot);
				player.getAppearance().generateAppearanceData();
				return;
