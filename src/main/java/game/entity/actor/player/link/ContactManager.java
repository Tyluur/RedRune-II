...
import utility.game.entity.actor.player.QuickChatMessage;
...
public class ContactManager implements Serializable {
	...
	public void changeFriendStatus(Player p2, boolean online) {
		...
		player.getPackets().sendFriend(Misc.formatPlayerNameForDisplay(p2.getUsername()), p2.getDisplayName(), 1, online, true);
	}
	
	public boolean isOnline(Player p2) {
		if (p2.getContactManager().privateStatus == 2) {
			return false;
		}
		return p2.getContactManager().privateStatus != 1 || p2.getContactManager().friends.contains(player.getUsername());
	}
	...
	public void changeRank(String username, int rank) {
		...
		player.getPackets().sendFriend(Misc.formatPlayerNameForDisplay(username), displayName, 1, online, true);
		...
		if (interfaceId == 1109) {
			if (componentId == 27) {
				...
				if (player.getInterfaceManager().containsScreenInter()) {
					player.getPackets().sendMessage("Please close the interface you have opened before using Friends Chat setup.");
					return;
				}
				player.stopAll();
				openFriendChatSetup();
	...
	public void addFriend(String username) {
		...
		if (username.equals(player.getUsername())) {
			player.getPackets().sendMessage("You can't add yourself.");
			return;
		}
		String displayName;
		Player p2 = World.getPlayerByDisplayName(username);
		...
		String formatedUsername = Misc.formatPlayerNameForProtocol(username);
		if (friends.contains(formatedUsername)) {
			player.getPackets().sendMessage((username) + " is already on your friends list.");
			return;
		}
		...
		FriendChatsManager.refreshChat(player);
		boolean online = p2 != null && isOnline(p2);
		player.getPackets().sendFriend(Misc.formatPlayerNameForDisplay(username), displayName, 1, online, online);
		if (privateStatus == 1 && p2 != null) {
			p2.getContactManager().changeFriendStatus(player, true);
		}
	}
	...
	public void init() {
		...
		for (String username : friends) {
			...
			player.getPackets().sendFriend(Misc.formatPlayerNameForDisplay(username), displayName, 1, p2 != null && isOnline(p2), false);
		}
		if (privateStatus != 2) {
			sendFriendsMyStatus(true);
		}
