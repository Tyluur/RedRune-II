...
import utility.game.entity.item.ItemSetsKeyGenerator;
...
public class BeastOfBurden implements Serializable {
	...
	public void addItem(int slot, int amount) {
		...
		if (!ItemConstants.isTradeable(item) || item.getId() == 4049 || (familiar.canStoreEssOnly() && item.getId() != 1436 && item.getId() != 7936) || item.getDefinitions().getValue() > 50000) {
			player.getPackets().sendMessage("You cannot store this item.");
			return;
		}
		Item[] itemsBefore = beastItems.getItemsCopy();
		...
		if (!item.getDefinitions().isStackable()) {
			if (freeSpace == 0) {
				player.getPackets().sendMessage("Not enough space in your Familiar Inventory.");
				return;
			}
