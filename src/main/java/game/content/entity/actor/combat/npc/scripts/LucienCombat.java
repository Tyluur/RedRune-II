		...
		if (Misc.getRandom(10) == 0) {
			...
			for (Actor t : possibleTargets) {
				if (t instanceof Player) {
					Player p = (Player) t;
					if (!p.getMusicManager().hasMusic(1008)) {
						p.getMusicManager().playMusic(581);
						p.getMusicManager().playMusic(584);
						p.getMusicManager().playMusic(579);
						p.getMusicManager().playMusic(1008);
						p.getPackets().sendMessage("You've received a reward while fighting Lucius!");
					}
				}
