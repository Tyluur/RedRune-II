...
public class PickPocketAction extends Action {
	...
	public boolean start(Player player) {
		if (checkAll(player)) {
			...
			player.getPackets().sendMessage("You attempt to pick the " + npc.getDefinitions().getName().toLowerCase() + "'s pocket...");
	...
	public int processWithDelay(Player player) {
		if (!isSuccesfull(player)) {
			player.getPackets().sendMessage("You fail to pick the " + npc.getDefinitions().getName().toLowerCase() + "'s pocket.");
			...
			player.getPackets().sendMessage("You've been stuned.");
			...
			player.getPackets().sendMessage("" + getMessage(player));
	...
	private boolean checkAll(Player player) {
		...
		if (player.getInventory().getFreeSlots() < 1) {
			player.getPackets().sendMessage("You don't have enough space in your inventory.");
			return false;
		}
		if (player.getAttackedBy() != null && player.getAttackedByDelay() > Misc.currentTimeMillis()) {
			player.getPackets().sendMessage("You can't do this while you're under combat.");
			return false;
		}
		if (npc.getAttackedBy() != null && npc.getAttackedByDelay() > Misc.currentTimeMillis()) {
			player.getPackets().sendMessage("The npc is under combat.");
			return false;
		}
		if (npc.isDead()) {
			player.getPackets().sendMessage("Too late, the npc is dead.");
			return false;
		}
		return true;
