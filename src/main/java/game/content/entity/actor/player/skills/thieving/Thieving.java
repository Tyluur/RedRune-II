		...
		for (final Stalls stall : Stalls.values()) {
			if (stall.getObjectId() == object.getId()) {
				...
				if (player.getSkills().getLevel(SkillConstants.THIEVING) < stall.getLevel()) {
					player.getPackets().sendMessage("You need a thieving level of " + stall.getLevel() + " to steal from this.", true);
					return;
				}
				if (player.getInventory().getFreeSlots() <= 0) {
					player.getPackets().sendMessage("Not enough space in your inventory.", true);
					return;
				}
	...
	public static void checkGuards(Player player) {
		...
		for (int regionId : player.getMapRegionsIds()) {
			...
			for (int npcIndex : npcIndexes) {
				...
				int distance = Misc.getDistance(npc.getX(), npc.getY(), player.getX(), player.getY());
