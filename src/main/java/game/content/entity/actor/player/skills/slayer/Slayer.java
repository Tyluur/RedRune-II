...
public class Slayer {
	...
	public static String assignTask(Player player, SlayerMaster master) {
		game.content.entity.actor.player.skills.slayer.SlayerTask tasks = player.getAttributes().getSlayerTask();
	...
	public static boolean checkRequirement(Player player, SlayerMonsters slayer) {
		if (slayer != null) {
			if (player.getSkills().getLevel(SkillConstants.SLAYER) < slayer.getRequirement()) {
				player.getPackets().sendMessage("This monster requires " + slayer.getRequirement() + " to slay.");
				return false;
			}
		}
		return true;
