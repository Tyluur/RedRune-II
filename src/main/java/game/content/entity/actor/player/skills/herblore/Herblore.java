package game.content.entity.actor.player.skills.herblore;
...
public class Herblore extends Action {
	...
	public boolean process(Player player) {
		...
		return player.getInventory().containsItem(node.getId(), node.getAmount()) && player.getInventory().containsItem(otherItem.getId(), otherItem.getAmount());
	}
	...
	public int processWithDelay(Player player) {
		...
		if (otherItem.getId() == VIAL || otherItem.getId() == COCONUT_MILK || node.getId() == VIAL || node.getId() == COCONUT_MILK) {
			player.getPackets().sendMessage("You add the " + node.getDefinitions().getName().toLowerCase().replace("clean", "") + " into the vial of " + (otherItem.getId() == VIAL ? "water." : "milk."));
		} else if (otherItem.getId() == SWAMP_TAR || node.getId() == SWAMP_TAR) {
			player.getPackets().sendMessage("You add the " + node.getDefinitions().getName().toLowerCase().replace("clean ", "") + " on the swamp tar.", true);
		} else if (otherItem.getId() == PESTLE_AND_MORTAR || node.getId() == PESTLE_AND_MORTAR) {
			player.getPackets().sendMessage("You crush the " + node.getDefinitions().getName().toLowerCase() + " with your pestle and mortar.", true);
		} else if (ingredients == Ingredients.TORSTOL && otherItem.getId() != VIAL) {
			player.getPackets().sendMessage("You combine the torstol with the potions and get an overload.");
			...
			player.getPackets().sendMessage("You mix the " + node.getDefinitions().getName().toLowerCase() + " into your potion.", true);
		}
