package game.content.entity.actor.player.skills.fishing;
...
public class Fishing extends Action {
	...
	public boolean start(Player player) {
		...
		player.getPackets().sendMessage("You attempt to capture a fish...");
	...
	private void addFish(Player player) {
		...
		player.getPackets().sendMessage(getMessage(fish));
	...
	private boolean checkAll(Player player) {
		...
		if (!player.getInventory().containsOneItem(spot.getTool())) {
			player.getPackets().sendMessage("You need a " + new Item(spot.getTool()).getDefinitions().getName().toLowerCase() + " to fish here.");
			return false;
		}
		if (!player.getInventory().containsOneItem(spot.getBait()) && spot.getBait() != -1) {
			player.getPackets().sendMessage("You don't have " + new Item(spot.getBait()).getDefinitions().getName().toLowerCase() + " to fish here.");
			return false;
		}
