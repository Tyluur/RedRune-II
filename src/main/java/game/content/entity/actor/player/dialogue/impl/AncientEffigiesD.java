...
import game.content.entity.item.AncientEffigies;
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			if (player.getSkills().getLevel((Integer) player.getTemporaryAttributes().get("skill1")) < AncientEffigies.getRequiredLevel(itemId)) {
				...
				player.getPackets().sendMessage("You're required at least " + AncientEffigies.getRequiredLevel(itemId) + " " + SkillConstants.SKILL_NAME[(Integer) player.getTemporaryAttributes().get("skill1")] + " to investigate this ancient effigy.");
			} else {
			...
			player.getPackets().sendMessage("You have gained " + AncientEffigies.getExp(itemId) + " " + SkillConstants.SKILL_NAME[(Integer) player.getTemporaryAttributes().get("skill")] + " experience!");
