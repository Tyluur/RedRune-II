package game.content.entity.actor.player.skills.runecrafting;
...
public final class Runecrafting {
	...
	public static void enterAirAltar(Player player) {
		player.getPackets().sendMessage("A mysterious force grabs hold of you.");
	...
	public static void craftEssence(Player player, int rune, int level, double experience, boolean pureEssOnly, int... multipliers) {
		...
		player.getPackets().sendMessage("You bind the temple's power into " + ItemDefinitions.getItemDefinitions(rune).getName().toLowerCase() + "s.");
	}
	...
	public static void locate(Player p, int xPos, int yPos) {
		...
		p.getPackets().sendMessage("The talisman pulls towards " + y + "-" + x + ".", false);
	}
	...
	public static void checkPouch(Player p, int i) {
		...
		p.getPackets().sendMessage("This pouch has " + p.getAttributes().getPouches()[i] + " rune essences in it.", false);
	}
	...
	public static void fillPouch(Player p, int i) {
		...
		if (LEVEL_REQ[i] > p.getSkills().getLevel(SkillConstants.RUNECRAFTING)) {
			p.getPackets().sendMessage("You need a runecrafting level of " + LEVEL_REQ[i] + " to fill this pouch.", false);
			return;
		}
		...
		if (!p.getInventory().containsOneItem(1436)) {
			p.getPackets().sendMessage("You don't have any essence with you.", false);
			return;
		}
		if (essenceToAdd == 0) {
			p.getPackets().sendMessage("Your pouch is full.", false);
			return;
		}
	}
	...
	public static void emptyPouch(Player p, int i) {
		...
		if (toAdd == 0) {
			p.getPackets().sendMessage("Your pouch has no essence left in it.", false);
		}
	}
}