package game.content.entity.actor.player.skills.summoning;
...
public class Summoning {
	...
	public static Familiar createFamiliar(Player player, Pouches pouch) {
		try {
			return (Familiar) Class.forName("game.entity.actor.npc.impl.familiar." + (NPCDefinitions.getNPCDefinitions(pouch.getNpcId())).getName().replace(" ", "").replace("-", "")).getConstructor(Player.class, Pouches.class, WorldTile.class, int.class, boolean.class).newInstance(player, pouch, player, -1, true);
		} catch (Throwable e) {
