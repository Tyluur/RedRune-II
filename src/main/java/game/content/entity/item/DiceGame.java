...
public class DiceGame {
	...
	private static void sendNumber(Player player, int numberRolled) {
		player.getPackets().sendMessage("Rolling...");// can be deleted if
		// you want
		player.setNextForceTalk(new ForceTalk("rolled <col=FF0000>" + numberRolled + "</col> " + "on the percentile dice"));
		player.getPackets().sendMessage("rolled <col=FF0000>" + numberRolled + "</col> " + "on the percentile dice");// correct message when
		// number is rolled
	}
