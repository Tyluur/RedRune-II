...
public class VengeanceGroupSpell implements RegularSpellPlugin {
	...
	public void cast(Player player, Actor target) {
		...
		if (player.getSkills().getLevel(MAGIC) < 94) {
			...
			player.getPackets().sendMessage("You need a Defence level of 40 for this spell");
			return;
		} else if (lastTimeCast != null && lastTimeCast + 30_000 > System.currentTimeMillis()) {
			player.getPackets().sendMessage("You must wait " + (TimeUnit.MILLISECONDS.toSeconds((lastTimeCast + 30_000) - System.currentTimeMillis())) + " more seconds to cast vengeance.");
			return;
		...
		player.getPackets().sendMessage("The spell affected " + affectedPeopleCount + " nearby people.");
