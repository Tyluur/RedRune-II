package org.redrune.game.content.entity.actor.player.skills.hunter;
...
public class Hunter extends Action {
    ...
    public boolean start(Player player) {
        ...
        player.getPackets().sendMessage("You start setting up the trap..");
    ...
    private boolean checkAll(Player player) {
        ...
        if (player.getAttributes().getTrapAmount() == getTrapAmount(player)) {
            player.getPackets().sendMessage("You can't setup more than " + player.getAttributes().getTrapAmount() + " traps.");
            return false;
        }
        ...
        if (objects != null) {
            for (WorldObject object : objects) {
                if (object.getX() == player.getX() && object.getY() == player.getY() && object.getPlane() == player.getPlane()) {
                    player.getPackets().sendMessage("You can't setup your trap here.");
                    return false;
                }
            }
        }
    ...
    public enum HunterNPC {
        ...
        CARNIVOROUS_CHINCHOMPA(5080, 10034, 63, 265, HunterEquipment.BOX, 28558),
        FERRT(5081, 10092, 27, 115, HunterEquipment.BOX, 19189),
        GECKO(6916, 12184, 27, 100, HunterEquipment.BOX, 19190),
        RACCOON(7272, 12487, 27, 100, HunterEquipment.BOX, 19191),
        MONKEY(6942, 12201, 27, 100, HunterEquipment.BOX, 28557),
        CRIMSON_SWIFT(5073, 10088, 1, 34, HunterEquipment.BRID_SNARE, 19180),
        GOLDEN_WARBLER(5075, 1583, 5, 48, HunterEquipment.BRID_SNARE, 19184),
        COPPER_LONGTAIL(5076, 10091, 9, 61, HunterEquipment.BRID_SNARE, 19186),
        CERULEAN_TWITCH(5074, 10089, 11, 64.67, HunterEquipment.BRID_SNARE, 19182),
        TROPICAL_WAGTAIL(5072, 10087, 19, 95.2, HunterEquipment.BRID_SNARE, 19178),
        WIMPY_BIRD(7031, 11525, 39, 167, HunterEquipment.BRID_SNARE, 28930);
