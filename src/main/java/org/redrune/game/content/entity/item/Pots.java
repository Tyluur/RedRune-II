...
public final class Pots {
	...
	public static boolean pot(Player player, Item item, int slot) {
		...
		player.getPackets().sendMessage(pot.effect.drinkMessage != null ? pot.effect.drinkMessage : "You drink some of your " + name.toLowerCase().replace(" (1)", "").replace(" (2)", "").replace(" (3)", "").replace(" (4)", "") + ".", true);
		
		player.getPackets().sendMessage(dosesLeft == 0 ? "You have finished your potion." : "You have " + dosesLeft + " dose of potion left.", true);
		return true;
	}
	...
	public static void resetOverLoadEffect(Player player) {
		...
		player.getPackets().sendMessage("The effects of overload have worn off.");
	}
		...
		RECOVER_SPECIAL() {
			...
			public boolean canDrink(Player player) {
				...
				if (time != null && Misc.currentTimeMillis() - time < 30000) {
					player.getPackets().sendMessage("You may only use this pot every 30 seconds.");
					return false;
				}
				return true;
			}
		...
		OVERLOAD() {
			...
			public boolean canDrink(Player player) {
				...
				if (player.getAttributes().getOverloadDelay() > 0) {
					player.getPackets().sendMessage("You may only use this potion every five minutes.");
					return false;
				}
