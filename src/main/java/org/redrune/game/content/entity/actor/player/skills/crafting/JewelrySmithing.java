...
public class JewelrySmithing {
					...
					player.getActionManager().setAction(new Action() {
						...
						public boolean process(Player player) {
							...
							if (player.getSkills().getLevel(SkillConstants.CRAFTING) < level) {
								player.getPackets().sendMessage("You need a Crafting level of " + level + ".");
								return false;
							} else if (!player.getInventory().containsItem(2357, 1)) {
								player.getPackets().sendMessage("You need a gold bar in order to create jewellery.");
								return false;
							} else if (actionSecondaryIndex != 0 && !player.getInventory().containsItem(GEMS[actionSecondaryIndex], 1)) {
								player.getPackets().sendMessage("You are missing the requried items in order to create this type of jewellery.");
								return false;
							}
							return true;
						}
	...
	public static void ringTransformation(Player player, final int itemId) {
		if (player.getActionManager().getAction() != null) {
			player.getPackets().sendMessage("Please finish what you are doing before transforming.");
			return;
		}
		player.getActionManager().setAction(new Action() {
