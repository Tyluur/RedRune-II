package org.redrune.game.content.entity.actor.player.skills.summoning;
...
public class Summoning {
	...
	public static void infusePouches(Player player) {
		...
		Object[] options = new Object[] { 78, 1, "List<col=FF9040>", "Infuse-X<col=FF9040>", "Infuse-All<col=FF9040>", "Infuse-10<col=FF9040>", "Infuse-5<col=FF9040>", "Infuse<col=FF9040>", 10, 8, INTERFACE << 16 | 16 };
	...
	public static void spawnFamiliar(Player player, Pouches pouch) {
		...
		if (skillReq != null) {
			for (int skillId : skillReq.keySet()) {
				...
				if (player.getSkills().getLevelForXp(skillId) < level) {
					if (hasRequiriments) {
						player.getPackets().sendMessage("You are not high enough level to use this pouch.");
					}
					hasRequiriments = false;
					String name = SkillConstants.SKILL_NAME[skillId].toLowerCase();
					player.getPackets().sendMessage("You need to have a" + (name.startsWith("a") ? "n" : "") + " " + name + " level of " + level + " to summon this.");
				}
			}
		}
		...
		if (npc == null) {
			player.getPackets().sendMessage("This familiar is not added yet.");
			return;
		}
	...
	public static Familiar createFamiliar(Player player, Pouches pouch) {
		try {
			return (Familiar) Class.forName("org.redrune.game.entity.actor.npc.impl.familiar." + (NPCDefinitions.getNPCDefinitions(pouch.getNpcId())).getName().replace(" ", "").replace("-", "")).getConstructor(Player.class, Pouches.class, WorldTile.class, int.class, boolean.class).newInstance(player, pouch, player, -1, true);
		} catch (Throwable e) {
		...
		if (skillReq != null) {
			for (int skillId : skillReq.keySet()) {
				...
				if (player.getSkills().getLevelForXp(skillId) < level) {
					...
					hasRequiriments = false;
					String name = SkillConstants.SKILL_NAME[skillId].toLowerCase();
					player.getPackets().sendMessage("You need to have a" + (name.startsWith("a") ? "n" : "") + " " + name + " level of " + level + " to create this.");
				}
			}
			HashMap<Integer, Integer> itemReq = def.getWearingSkillRequirements();
			if (itemReq != null) {
				for (int reqId : itemReq.keySet()) {
					...
					if (!player.getInventory().containsItem(reqId, amount)) {
						...
						player.getPackets().sendMessage("You need to have a" + (name.startsWith("a") ? "n" : "") + " " + name + " X " + amount + ".");
					}
	...
	public enum Pouches {
		...
		BEAVER(9475, 89, 12021, 33, 0.7, 57.6, 1620000, 4, 12429),
		// TODO
		...
		BULL_ANT(6969, 91, 12087, 40, 0.6, 52.8, 1800000, 5, 12431),
		// TODO
