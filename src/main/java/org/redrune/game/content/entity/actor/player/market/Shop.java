...
public final class Shop {
     ...
     * @param amount The amount of the item we wish to buy
     */
    public void buy(Player player, int slotId, int amount) {
        ...
        if (maxQuantity >= buyQuantity) {
            ...
            player.getPackets().sendMessage("You need " + Misc.format(buyPrice * amount) + " " + currency.name() + " to buy this item.");
     ...
     * @param player The player
     * @param slotId The slot the item is in
     * @param amount The amount we wish to sell
     */
    public void sell(Player player, int slotId, int amount) {
        ...
        if (!sellable(item.getId())) {
            player.getPackets().sendMessage("You can't sell this item to the shop.", true);
            return;
        }
        ...
        if (!player.getInventory().getItems().hasSpaceForItem(currencyItem)) {
            player.getPackets().sendMessage("Not enough space in your inventory.", false);
            return;
        }
        ...
        if (stockAmount == 1) {
            ...
            if (sellAmount == 0) {
                player.getPackets().sendMessage("This item can only be sold in increments of " + stockAmount + ".", false);
                return;
            }
     ...
     * @param inventory If the item is in the inventory or the shop stock
     */
    public void value(Player player, int slotId, boolean inventory) {
        ...
        if (inventory && !sellable(item.getId())) {
            player.getPackets().sendMessage("You can't sell this item to the shop.", false);
            return;
        }
        ...
        if (inventory) {
            player.getPackets().sendMessage(item.getName() + " will sell for " + (sellPrice == 0 ? "FREE" : Misc.format(sellPrice) + " " + currency.name() + "") + ".", false);
        } else {
            player.getPackets().sendMessage((amount == 1 ? "" : Misc.format(amount) + "x ") + item.getName() + ": shop will sell for " + (buyPrice == 0 ? "FREE" : Misc.format(buyPrice)) + " " + currency.name() + ".", false);
        }
    }
    ...
    public void open(Player player) {
        ...
        player.putTemporaryAttribute("open_shop", this);
