...
public class Foods {
    ...
    public static boolean eat(final Player player, Item item, int slot) {
        ...
        if (player.getAttributes().getFoodDelay() > Misc.currentTimeMillis()) {
            if (food.equals(Food.KARAMBWANI)) {
                if (player.getTemporaryAttribute("karambwani_eat") != null) {
                    if (System.currentTimeMillis() - (long) player.getTemporaryAttribute("karambwani_eat") <= 800) {
                        return true;
                    }
                }
                player.putTemporaryAttribute("karambwani_eat", System.currentTimeMillis());
            }
            return true;
        }
        player.getPackets().sendMessage("You eat the " + ItemDefinitions.getItemDefinitions(food.getId()).getName().toLowerCase() + ".");
        ...
        if (player.getHitpoints() > hp) {
            player.getPackets().sendMessage("It heals some health.");
        }
    ...
    public enum Effect {
        ...
        CABAGE_MESSAGE {
            public void effect(Object object) {
                Player player = (Player) object;
                player.getPackets().sendMessage("You don't really like it much.", true);
            }
        },
        ...
        ONION_MESSAGE {
            public void effect(Object object) {
                Player player = (Player) object;
                player.getPackets().sendMessage("It hurts to see a grown " + player.getAppearance().isMale() != null ? "male" : "female" + "cry.");
            }
        },
