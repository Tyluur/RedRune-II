		...
		if (player != null) {
			...
			switch (Misc.getRandom(3)) {
				case 0: // Melee
					if (npc.withinDistance(target, 3)) {
						...
						if (CombatAlgorithm.hasAntiDragProtection(target) || (player != null && (player.getPrayer().usingPrayer(0, 17) || player.getPrayer().usingPrayer(1, 7)))) {
							damage = 0;
							player.getPackets().sendMessage("Your " + (CombatAlgorithm.hasAntiDragProtection(target) ? "shield" : "prayer") + " absorbs most of the dragon's breath!", true);
						} else if ((!CombatAlgorithm.hasAntiDragProtection(target) || !player.getPrayer().usingPrayer(0, 17) || !player.getPrayer().usingPrayer(1, 7)) && player.getAttributes().getFireImmune() > Misc.currentTimeMillis()) {
							damage = Misc.getRandom(164);
							player.getPackets().sendMessage("Your potion absorbs most of the dragon's breath!", true);
						}
				...
				case 1: // Dragon breath
					if (npc.withinDistance(target, 3)) {
						...
						if (CombatAlgorithm.hasAntiDragProtection(target) || (player != null && (player.getPrayer().usingPrayer(0, 17) || player.getPrayer().usingPrayer(1, 7)))) {
							...
							damage = Misc.getRandom(164);
							player.getPackets().sendMessage("Your potion fully protects you from the heat of the dragon's breath!", true);
						}
				...
				case 2: // Range
					...
					npc.setNextAnimation(new Animation(13155));
					RegionManager.sendProjectile(npc, target, 2707, 28, 16, 35, 35, 16, 0);
					delayHit(npc, 1, target, getRangeHit(npc, damage));
					break;
				case 3: // Ice arrows range
					damage = Misc.getRandom(250);
					npc.setNextAnimation(new Animation(13155));
					RegionManager.sendProjectile(npc, target, 369, 28, 16, 35, 35, 16, 0);
					delayHit(npc, 1, target, getRangeHit(npc, damage));
					break;
				case 4: // Orb crap
					break;
