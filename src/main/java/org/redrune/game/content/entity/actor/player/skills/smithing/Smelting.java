package org.redrune.game.content.entity.actor.player.skills.smithing;
...
public class Smelting extends Action {
	...
	public boolean start(Player player) {
		...
		player.getPackets().sendMessage("You place the required ores and attempt to create a bar of " + bar.getProducedBar().getDefinitions().getName().toLowerCase().replace(" bar", "") + ".", true);
		return true;
	}
	...
	public int processWithDelay(Player player) {
		...
		if (isSuccessFull(player)) {
			...
			player.getPackets().sendMessage("You retrieve a bar of " + bar.getProducedBar().getDefinitions().getName().toLowerCase().replace(" bar", "") + ".", true);
		} else {
			player.getPackets().sendMessage("The ore is too impure and you fail to refine it.", true);
		}
		if (ticks > 0) {
			return 1;
		}
