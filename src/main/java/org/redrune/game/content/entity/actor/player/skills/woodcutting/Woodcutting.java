package org.redrune.game.content.entity.actor.player.skills.woodcutting;
...
public final class Woodcutting extends Action {
    ...
    public boolean start(Player player) {
        ...
        player.getPackets().sendMessage(usingBeaver ? "Your beaver uses its strong teeth to chop down the tree..." : "You swing your hatchet at the " + (TreeDefinitions.IVY == definitions ? "ivy" : "tree") + "...", true);
    ...
    private boolean checkAll(Player player) {
        if (!hasAxe(player)) {
            player.getPackets().sendMessage("You need a hatchet to chop down this tree.");
            return false;
        }
        if (!setAxe(player)) {
            player.getPackets().sendMessage("You dont have the required level to use that axe.");
            return false;
        }
    ...
    private int getWoodcuttingDelay(Player player) {
        ...
        int wcTimer = definitions.getLogBaseTime() - (player.getSkills().getLevel(8) + summoningBonus) - Misc.getRandom(axeTime);
    ...
    private boolean hasWoodcuttingLevel(Player player) {
        if (definitions.getLevel() > player.getSkills().getLevel(8)) {
            player.getPackets().sendMessage("You need a woodcutting level of " + definitions.getLevel() + " to chop down this tree.");
            return false;
        }
        return true;
    }
    ...
    public int processWithDelay(Player player) {
        ...
        if (!usedDeplateAurora && (1 + Math.random()) < player.getAuraManager().getChanceNotDepleteMN_WC()) {
            ...
            long time = definitions.respawnDelay * 600L;
    ...
    private void addLog(Player player) {
        ...
        if (definitions == TreeDefinitions.IVY) {
            player.getPackets().sendMessage("You succesfully cut an ivy vine.", true);
            // todo gfx
        } else {
            String logName = ItemDefinitions.getItemDefinitions(definitions.getLogsId()).getName().toLowerCase();
            player.getPackets().sendMessage("You get some " + logName + ".", true);
            // todo infernal adze
        }
    }
