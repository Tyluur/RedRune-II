package org.redrune.game.content.entity.actor.player.skills.cooking;
...
public class Cooking extends Action {
    ...
    public boolean start(Player player) {
        ...
        player.getPackets().sendMessage("You attempt to cook the " + cook.getProduct().getDefinitions().getName().toLowerCase() + ".");
    ...
    public int processWithDelay(Player player) {
        ...
        if ((player.getSkills().getLevel(SkillConstants.COOKING) < cook.getBurningLvl()) && isBurned(player)) {
            ...
            player.getPackets().sendMessage("Oops! You accidently burnt the " + cook.getProduct().getDefinitions().getName().toLowerCase() + ".", true);
        } else {
            player.getInventory().deleteItem(item.getId(), 1);
            ...
            player.getPackets().sendMessage("You successfully cook the " + cook.getProduct().getDefinitions().getName().toLowerCase() + ".", true);
        }
        if (amount > 0) {
            player.getPackets().sendMessage("You attempt to cook the " + cook.getProduct().getDefinitions().getName().toLowerCase() + ".");
            return 1;
