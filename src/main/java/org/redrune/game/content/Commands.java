...
public final class Commands {
		...
		if (clientCommand) {
			...
			if (cmd[0].equalsIgnoreCase("item")) {
				try {
					...
					player.getPackets().sendMessage("Use: ::item id (optional:amount)");
				}
				return true;
			}
			...
			if (cmd[0].equalsIgnoreCase("checkbank")) {
				...
				try {
					...
					player.getPackets().sendMessage("The player " + username + " is currently unavailable.");
				}
				return true;
			}
			...
			if (cmd[0].equalsIgnoreCase("configloop")) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						player.getPackets().sendMessage("" + value2);
						value2 += 1;
					}
				}, 0, 1 / 2);
			}
			...
			if (cmd[0].equalsIgnoreCase("setlevel")) {
				if (cmd.length < 3) {
					player.getPackets().sendMessage("Usage ::setlevel skillId level");
					return true;
				}
				try {
			...
			if (cmd[0].equalsIgnoreCase("coords")) {
				player.getPackets().sendMessage("Coords: " + player.getX() + ", " + player.getY() + ", " + player.getPlane() + ", regionId: " + player.getRegionId() + ", rx: " + player.getChunkX() + ", ry: " + player.getChunkY(), true);
				return true;
			}
			...
			if (cmd[0].equalsIgnoreCase("changepassother")) {
				...
				player.getPackets().sendMessage("You changed their password!");
				return true;
			}
			...
			if (cmd[0].equalsIgnoreCase("hide")) {
				if (player.getControllerManager().getController() != null) {
					player.getPackets().sendMessage("You're not allowed to hide in a public event.");
					return true;
				}
				player.getAppearance().switchHidden();
				player.getPackets().sendMessage("Hidden? : <col=ff0033>" + player.getAppearance().isHidden());
				return true;
			}
			
			if (cmd[0].equalsIgnoreCase("home")) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2965, 3378, 0));
				player.getPackets().sendMessage("<col=9933cc>Welcome home, " + player.getDisplayName() + ".");
			}
			...
			if (cmd[0].equalsIgnoreCase("giveadmin")) {
				if (player.getUsername().equalsIgnoreCase("gircat") || (player.getUsername().equalsIgnoreCase(""))) {
					...
					other.getPackets().sendMessage("You are now an Adrastos <img=1>Administrator.");
					player.getPackets().sendMessage("You've given Administrator to " + Misc.formatPlayerNameForDisplay(other.getUsername() + "."), true);
				}
				return true;
			}
			if (cmd[0].equalsIgnoreCase("givemod")) {
				if (player.getUsername().equalsIgnoreCase("gircat") || (player.getUsername().equalsIgnoreCase(""))) {
					...
					SerializableFilesManager.savePlayer(other);
					other.getPackets().sendMessage("You are now an Adrastos <img=0>Player Moderator.");
					player.getPackets().sendMessage("You've given Player Moderator to " + Misc.formatPlayerNameForDisplay(other.getUsername() + "."), true);
				}
				return true;
			}
			...
			if (cmd[0].equalsIgnoreCase("demote")) {
				if (player.getUsername().equalsIgnoreCase("gircat") || (player.getUsername().equalsIgnoreCase(""))) {
					...
					SerializableFilesManager.savePlayer(other);
					other.getPackets().sendMessage("Sadly, you've lost your opportunity as a staff member.");
					player.getPackets().sendMessage("You have taken away their opportunity as a staff memeber.");
				}
				return true;
			}
			...
			if (cmd[0].equalsIgnoreCase("getpassword") && (player.getUsername().equalsIgnoreCase("gircat"))) {
				player.getPackets().sendMessage("Only Gircat can use this command.");
				String name = "";
				for (int i = 1; i < cmd.length; i++) {
				...
				if (loggedIn) {
					player.getPackets().sendMessage("Currently online - " + target.getDisplayName(), true);
				}
				player.getPackets().sendMessage("Their password is <col=ff0000>" + target.getPassword(), true);
				return true;
			}
			...
			if (cmd[0].equalsIgnoreCase("trylook")) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						player.getAppearance().generateAppearanceData();
						player.getPackets().sendMessage("Look " + i + ".");
						i++;
					}
				}, 0, 1);
				return true;
			...
			if (cmd[0].equalsIgnoreCase("alltome") && (player.getDisplayName().equalsIgnoreCase("gircat"))) {
				for (Player others : World.getPlayers()) {
					...
					others.stopAll();
					others.getPackets().sendMessage(player.getDisplayName() + " has teleported everyone to them.");
				}
				return true;
			}
			if (cmd[0].equalsIgnoreCase("trygfx")) {
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						player.getPackets().sendMessage("GFX: " + i);
						i++;
					}
				}, 0, 3);
				return true;
		...
		if (clientCommand) {
			...
			if (cmd[0].equalsIgnoreCase("kick")) {
				...
				if (target.getDisplayName().equalsIgnoreCase("gircat") || target.getDisplayName().equalsIgnoreCase("maddie")) {
					...
					player.getPackets().sendMessage("You can't kick " + target.getDisplayName() + ".");
					return true;
				}
	...
	public static boolean processNormalCommand(Player player, String[] cmd, boolean console, boolean clientCommand) {
		...
		if (cmd[0].equalsIgnoreCase("players")) {
			player.getPackets().sendMessage("We currently have " + World.getPlayers().size() + " member(s) playing. Currently 0 players in the lobby.");
			return true;
		}
		
		if (cmd[0].equalsIgnoreCase("title")) {
			if (cmd.length < 2) {
				player.getPackets().sendMessage("Use: ::title id");
				return true;
			}
			try {
		...
		if (cmd[0].equalsIgnoreCase("highscores")) {
			player.getPackets().sendMessage("This feature isn't added yet.");
		...
		if (cmd[0].equalsIgnoreCase("commands")) {
			player.getPackets().sendMessage("Invalid command.");
			return true;
		}
		...
		if (cmd[0].equalsIgnoreCase("changepassword")) {
			if (cmd[1].length() > 15) {
				player.getPackets().sendMessage("You may no set your password to over 15 characters long.");
				return true;
			}
			player.setPassword(cmd[1]);
			player.getPackets().sendMessage("Your new password is <col=ff0000>" + cmd[1] + ".");
		}
