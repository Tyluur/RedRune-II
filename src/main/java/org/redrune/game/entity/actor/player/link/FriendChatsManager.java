...
public class FriendChatsManager {
	...
	public static void destroyChat(Player player) {
		synchronized (cachedFriendChats) {
			...
			chat.destroyChat();
			player.getPackets().sendMessage("Your friends chat channel has now been disabled!");
		}
	}
	...
	public void destroyChat() {
		synchronized (this) {
			for (Player player : players) {
				...
				player.getPackets().sendMessage("You have been removed from this channel!");
			}
		}
		synchronized (cachedFriendChats) {
	...
	public static void joinChat(String ownerName, Player player) {
		synchronized (cachedFriendChats) {
			...
			player.getPackets().sendMessage("Attempting to join channel...");
			...
			if (chat == null) {
				...
				if (owner == null) {
					if (!PlayerSaving.playerExists(formatedName)) {
						player.getPackets().sendMessage("The channel you tried to join does not exist.");
						return;
					}
				...
				if (!player.getUsername().equals(ownerName) && !settings.hasRankToJoin(player.getUsername())) {
					player.getPackets().sendMessage("You do not have a enough rank to join this friends chat channel.");
					return;
				}
	...
	private void joinChat(Player player) {
		synchronized (this) {
			...
			if (players.size() >= 100) {
				player.getPackets().sendMessage("This chat is full.");
				return;
			}
			Long bannedSince = bannedPlayers.get(player.getUsername());
			if (bannedSince != null) {
				if (bannedSince + 3600000 > Misc.currentTimeMillis()) {
					player.getPackets().sendMessage("You have been banned from this channel.");
					return;
				}
	...
	public void leaveChat(Player player, boolean logout) {
		synchronized (this) {
			...
			if (!logout) {
				player.getAttributes().setCurrentFriendChatOwner(null);
				player.getPackets().sendMessage("You have left the channel.");
				player.getPackets().sendFriendsChatChannel();
			}
		}
	}
	...
	private void joinChatNoCheck(Player player) {
		synchronized (this) {
			...
			player.getPackets().sendMessage("You are now talking in the friends chat channel " + settings.getChatName());
			player.getPackets().sendMessage("To talk, start each line of chat with the / symbol.");
			refreshChannel();
		}
	}
	...
	public void kickPlayerFromChat(Player player, String username) {
		...
		synchronized (this) {
			...
			if (kicked == null) {
				player.getPackets().sendMessage("This player is not this channel.");
				return;
			}
			...
			kicked.getPackets().sendMessage("You have been kicked from the friends chat channel.");
			player.getPackets().sendMessage("You have kicked " + kicked.getUsername() + " from friends chat channel.");
			refreshChannel();
		}
	}
	...
	private void sendMessage(Player player, ChatMessage message) {
		synchronized (this) {
			if (!player.getUsername().equals(owner) && !settings.canTalk(player)) {
				player.getPackets().sendMessage("You do not have a enough rank to talk on this friends chat channel.");
				return;
			}
