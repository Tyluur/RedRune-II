...
public class PlayerBank implements Serializable {
    ...
    public void withdrawItemButOne(int fakeSlot) {
        ...
        if (item.getAmount() <= 1) {
            player.getPackets().sendMessage("You only have one of this item in your bank");
            return;
        }
    ...
    public void depositAllInventory(boolean banking) {
        if (PlayerBank.MAX_BANK_SIZE - getBankSize() < player.getInventory().getItems().getSize()) {
    ...
    public void withdrawItem(int bankSlot, int quantity) {
        ...
        if (withdrawNotes) {
            if (!defs.isNoted() && defs.getCertId() != -1) {
                ...
                player.getPackets().sendMessage("You cannot withdraw this item as a note.");
            }
        }
