...
public final class ActionLocks {
	 ...
	 */
	public void unlock(String key, Actor actor) {
		unlock(key, true, actor);
	}
	 ...
	 *
	 * @param cacheRemove
	 * 		If the lock can be removed from the cache.
	 */
	public void unlock(String key, boolean cacheRemove, Actor actor) {
		Lock lock = customLocks.get(key);
		if (lock != null) {
			lock.unlock();
			if (lock.getElapse() != null) {
				lock.getElapse().elapse(actor, lock);
			}
			if (cacheRemove) {
