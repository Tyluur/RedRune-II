...
import org.redrune.game.content.entity.actor.player.skills.fishing.Fishing.Fish;
        ...
        for (Fish fish : Fish.values()) {
            if (fish.getId() == item.getId()) {
                if (getOwner().getSkills().getLevel(SkillConstants.COOKING) < fish.getLevel()) {
                    getOwner().getPackets().sendMessage("Your cooking level is not high enough for the bunyip to eat this fish.");
                    return false;
                } else {
                ...
                getOwner().getPackets().sendMessage("Your bunyip cannot eat this.");
                return false;
            }
        }
        return true;
