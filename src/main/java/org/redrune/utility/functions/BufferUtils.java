...
public class BufferUtils {
    ...
    public static byte[] decrypt(int[] cryption, byte[] data, int offset, int length) {
        ...
        for (int i = 0; i < numBlocks; i++) {
            ...
            do {
                dataBlock[1] -= (cryption[(int) ((sum & 0x1933) >>> 11)] + sum ^ dataBlock[0] + ((long) dataBlock[0] << 4 ^ dataBlock[0] >>> 5));
                sum -= DELTA;
                dataBlock[0] -= (((long) dataBlock[1] << 4 ^ dataBlock[1] >>> 5) + dataBlock[1] ^ cryption[(int) (sum & 0x3)] + sum);
                round++;
            } while (round < 32);
