    ...
    override fun read(player: Player, stream: Packet): PacketContext? {
        ...
        when (packetId) {
            ...
            PacketConstants.SEND_FRIEND_MESSAGE_PACKET -> {
                ...
                val p2 = World.getPlayerByDisplayName(username) ?: return null
                return object : PacketContext() {
                    override fun handle(player: Player) {
            ...
            PacketConstants.PUBLIC_QUICK_CHAT_PACKET -> {
                ...
                if (player.attributes.lastPublicMessage > Misc.currentTimeMillis()) {
                    return null
                }
            ...
            PacketConstants.CHAT_PACKET -> {
                ...
                val `is` = InputStream(stream.buffer.array())
                val colorEffect = `is`.readUnsignedByte()
                val moveEffect = `is`.readUnsignedByte()
                val message = Huffman.readEncryptedMessage(250, `is`)
                if (message.replace(" ".toRegex(), "") == "") {
                    return null
                }
