 ...
 * stream.
 *
 * <p>The decompression requires large amounts of memory. Thus you should call the {@link #close() close()} method as
 * soon as possible, to force <tt>CBZip2InputStream</tt> to release the allocated memory.  See {@link CBZip2OutputStream
 * CBZip2OutputStream} for information about memory usage.</p>
 *
 * <p><tt>CBZip2InputStream</tt> reads bytes from the compressed source stream via the single byte {@link
 * InputStream#read() read()} method exclusively. Thus you should consider to use a buffered source stream.</p>
 *
...
public class CBZip2InputStream extends InputStream implements BZip2Constants {
	 ...
	 *
	 * <p>Although BZip2 headers are marked with the magic <tt>"Bz"</tt> this constructor expects the next byte in the
	 * stream to be the first one after the magic.  Thus callers have to skip the first two bytes. Otherwise this
	...
	private static final class Data extends Object {
		 ...
		 *
		 * This method is called when the required length of the array is known.  I don't initialize it at construction
		 * time to avoid unneccessary memory allocation when compressing small files.
		 */
