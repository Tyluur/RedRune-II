...
public class ProjectilePathFinder {
	 ...
	 * The solid tile flag.
	 */
	private static final int SOLID_FLAG = 0x100;
	
	/**
	 * The north clipped tile flag.
	 */
	private static final int NORTH_FLAG = 0x48240000;
	
	/**
	 * The south clipped tile flag.
	 */
	private static final int SOUTH_FLAG = 0x40a40000;
	
	/**
	 * The east clipped tile flag.
	 */
	private static final int EAST_FLAG = 0x60240000;
	
	/**
	 * The west clipped tile flag.
	 */
	private static final int WEST_FLAG = 0x42240000;
	...
	public static boolean findPath(Entity e, Entity opponent) {
		...
		if (start.getPlane() != dest.getPlane()) {
			return false;
		}
		if (start == dest) {
			return true;
		...
		for (Location l : getPath(start, dest, start.getPlane())) {
			int clippingMask = RegionManager.getClippingMask(l.getX(), l.getY(), l.getPlane());
	...
	public static List<Location> getPath(Location start, Location dest, int z) {
		...
		for (int i = 0; i < max; i++) {
			...
			tiles.add(Location.create((endX - diffX), (endY - diffY), z));
		}
		return tiles;
	}
