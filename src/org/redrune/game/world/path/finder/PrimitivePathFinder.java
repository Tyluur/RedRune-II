package org.redrune.game.world.path.finder;

import org.redrune.game.node.Location;
import org.redrune.game.node.entity.Entity;
import org.redrune.game.world.path.PathFinder;
import org.redrune.game.world.path.PathState;
import org.redrune.game.world.region.RegionManager;
import org.redrune.utility.rs.Position;
import org.redrune.utility.rs.constant.Directions;
...
public class PrimitivePathFinder implements PathFinder {
	...
	@Override
	public PathState findPath(Entity entity, Location base, int srcX, int srcY, int dstX, int dstY, int z, int radius, boolean running, boolean ignoreLastStep, boolean moveNear) {
		return findPath(entity, base, srcX, srcY, dstX, dstY, z, radius, running, ignoreLastStep, moveNear, false);
	}
	...
	public PathState findPath(Entity mob, Location base, int srcX, int srcY, int dstX, int dstY, int z, int radius, boolean running, boolean ignoreLastStep, boolean moveNear, boolean nullOnFail) {
		...
		for (int i = 0; i < stepCount; i++) {
			Location curLoc = Location.create(location.getX() + curX, location.getY() + curY, location.getPlane());
			...
			if (dstDir != null && canMove(curLoc, dstDir, false)) {
				...
				if (!ignoreLastStep || curX != dstX || curY != dstY) {
					state.getPoints().add(new Position(step.getX(), step.getY(), step.getPlane()));
				} else {
					break;
				}
			} else if (nullOnFail) {
	...
	private static boolean canMove(Location source, Directions.WalkingDirection dir, boolean diagonalCheck) {
		return canMove(source, dir, diagonalCheck, false);
	}
	
	private static boolean canMove(Location source, Directions.WalkingDirection dir, boolean diagonalCheck, boolean npcCheck) {
		if (dir == null) {
			return true;
		}
		// TODO
		// if(!diagonalCheck) Make it so they only get stuck on the following directions: NORTH,  SOUTH, EAST, WEST
	...
	private static boolean canMove(Location base, int startX, int startY, int endX, int endY, int height, int xLength, int yLength) {
		Location location = Location.create((base.getRegionX() - 6) << 3, (base.getRegionY() - 6) << 3, base.getPlane());
		...
		for (int ii = 0; ii < max; ii++) {
			...
			for (int i = 0; i < xLength; i++) {
				for (int i2 = 0; i2 < yLength; i2++) {
					if (diffX < 0 && diffY < 0) {
						if ((RegionManager.getClippingMask(currentX + i - 1, currentY + i2 - 1, height) & 0x128010e) != 0 || (RegionManager.getClippingMask(currentX + i - 1, currentY + i2, height) & 0x1280108) != 0 || (RegionManager.getClippingMask(currentX + i, currentY + i2 - 1, height) & 0x1280102) != 0) {
							return false;
						}
					} else if (diffX > 0 && diffY > 0) {
						if ((RegionManager.getClippingMask(currentX + i + 1, currentY + i2 + 1, height) & 0x12801e0) != 0 || (RegionManager.getClippingMask(currentX + i + 1, currentY + i2, height) & 0x1280180) != 0 || (RegionManager.getClippingMask(currentX + i, currentY + i2 + 1, height) & 0x1280120) != 0) {
							return false;
						}
					} else if (diffX < 0 && diffY > 0) {
						if ((RegionManager.getClippingMask(currentX + i - 1, currentY + i2 + 1, height) & 0x1280138) != 0 || (RegionManager.getClippingMask(currentX + i - 1, currentY + i2, height) & 0x1280108) != 0 || (RegionManager.getClippingMask(currentX + i, currentY + i2 + 1, height) & 0x1280120) != 0) {
							return false;
						}
					} else if (diffX > 0 && diffY < 0) {
						if ((RegionManager.getClippingMask(currentX + i + 1, currentY + i2 - 1, height) & 0x1280183) != 0 || (RegionManager.getClippingMask(currentX + i + 1, currentY + i2, height) & 0x1280180) != 0 || (RegionManager.getClippingMask(currentX + i, currentY + i2 - 1, height) & 0x1280102) != 0) {
							return false;
						}
					} else if (diffX > 0 && diffY == 0) {
						if ((RegionManager.getClippingMask(currentX + i + 1, currentY + i2, height) & 0x1280180) != 0) {
							return false;
						}
					} else if (diffX < 0 && diffY == 0) {
						if ((RegionManager.getClippingMask(currentX + i - 1, currentY + i2, height) & 0x1280108) != 0) {
							return false;
						}
					} else if (diffX == 0 && diffY > 0) {
						if ((RegionManager.getClippingMask(currentX + i, currentY + i2 + 1, height) & 0x1280120) != 0) {
							return false;
						}
					} else if (diffX == 0 && diffY < 0) {
						if ((RegionManager.getClippingMask(currentX + i, currentY + i2 - 1, height) & 0x1280102) != 0) {
							return false;
						}
					}
				}
