...
public final class RegionBuilder {
	...
	public static DynamicRegion createDynamicRegion(int regionId) {
		synchronized (ALGORITHM_LOCK) {
			...
			if (region != null) {
				// if its already dynamic lets keep building it
			...
			RegionManager.getRegions().put(regionId, new DynamicRegion(regionId));
			return new DynamicRegion(regionId);
		}
	}
	...
	public static void destroyRegion(int regionId) {
		Region region = RegionManager.getRegions().get(regionId);
		if (region != null) {
			CopyOnWriteArraySet<Player> playerList = region.getPlayers();
			CopyOnWriteArraySet<NPC> npcList = region.getNpcs();
			...
			for (Player player : playerList) {
				...
				player.putAttribute(AttributeKey.FORCE_NEXT_MAP_LOAD, true);
