...
public class RegionBuilder {
	...
	public static GameObject addObject(GameObject object, boolean defaultObject) {
		...
		if (object.getType() == 22) {
			if (def.getActionCount() == 1) {
				addClipping(object.getLocation().getX(), object.getLocation().getY(), object.getLocation().getPlane(), 0x200000);
			}
		} else if (object.getType() >= 9 && object.getType() <= 11) {
			if (def.getActionCount() != 0) {
				addClippingForSolidObject(object.getLocation().getX(), object.getLocation().getY(), object.getLocation().getPlane(), xLength, yLength, def.isSolid(), !def.isClippingFlag());
			}
	...
	public static void addClipping(int x, int y, int z, int shift) {
		...
		if (region.getClippingMasks()[z] == null) {
			region.getClippingMasks()[z] = new int[RegionConstants.REGION_SIZE][RegionConstants.REGION_SIZE];
		}
