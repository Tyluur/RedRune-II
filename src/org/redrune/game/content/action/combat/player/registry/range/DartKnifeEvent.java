...
public class DartKnifeEvent implements BowFireEvent {
	...
	public void fire(Player attacker, Entity target, CombatTypeSwing swing, int weaponId, int ammoId) {
		...
		if (name.contains("knife")) {
			...
			attacker.getRegion().sendProjectile(ProjectileManager.createRangeProjectile(attacker, target, StaticCombatFormulae.getKnifeThrowGfxId(weaponId), 40, 36, 32, 15, 0));
		}
		
		dropAmmo(attacker, target.getLocation(), EquipConstants.SLOT_WEAPON, weaponId, false);
	}
}
