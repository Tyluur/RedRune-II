package org.redrune.game.content.action.impl;
...
public class PlayerCombatAction extends Action {
	...
	public int processWithDelay(Player player) {
		...
		if (delay == -1) {
			player.getPackets().sendGameMessage((style == CombatStyle.MAGIC ? "Spell #" + id + "" : "Weapon #" + weaponId) + " has not yet been added, please report this on the forums.");
		...
		if (!(target.isNPC() && target.toNPC().isForceMultiAttacked())) {
			if (!target.isAtMultiArea() || !player.isAtMultiArea()) {
				...
				if (target.getAttackedBy() != player && target.getAttackedByDelay() > System.currentTimeMillis()) {
					player.getPackets().sendGameMessage((target.isPlayer() ? "That player is" : "This npc is") + " already in combat");
					return false;
				}
			}
		}
