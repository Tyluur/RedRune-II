...
public class ItemDefinitionParser {
	...
	private static final ConcurrentHashMap<Integer, ItemDefinitionParser> ITEM_DEFINITIONS = new ConcurrentHashMap<>();
	...
	public static ItemDefinitionParser getItemDefinition(int itemId) {
		ItemDefinitionParser definitions = ITEM_DEFINITIONS.get(itemId);
		if (definitions != null) {
			return definitions;
		} else {
			ItemDefinitionParser def = new ItemDefinitionParser(itemId);
...
/*	public static void loadEquipIds() {
	...
	private ItemDefinitionParser(int id) {
		this.id = id;
	}
	...
	private void loadItemDefinition() {
		...
		if (clientScriptData == null) { return; }
		...
		for (int opcode2 : clientScriptData.keySet()) {
			...
			if (opcode2 >= 749 && opcode2 < 797) {
				...
				if (levelId != -1 && levelReq != -1) {
					...
					} else if (id == 7462) { levelRequirements.put(Skills.DEFENCE, 45); } else if (id == 20072) {
			...
			for (int reqOpcode : ITEM_REQS) {
				if (opcode2 == reqOpcode) {
					...
					if (itemRequirements == null) { itemRequirements = new HashMap<>(); }
			...
			switch (opcode2) {
				case 394:
					if (levelRequirements == null) { levelRequirements = new HashMap<>(); }
	...
	private void transferNoteDefinition(ItemDefinitionParser reference, ItemDefinitionParser templateReference) {
	...
	private void transferLendDefinition(ItemDefinitionParser reference, ItemDefinitionParser templateReference) {
		...
		if (reference.inventoryOptions != null) { inventoryOptions = reference.inventoryOptions.clone(); }
	}
	
	private void transferRecolourDefinition(ItemDefinitionParser reference, ItemDefinitionParser templateReference) {
		femaleWornModelId2 = reference.femaleWornModelId2;
		inventoryOptions = new String[5];
		modelRotation2 = templateReference.modelRotation2;
		name = reference.name;
	...
	public int getGroupId() {
		...
		if (specialBar != null && specialBar instanceof Integer) { return (Integer) specialBar; }
		return 0;
	}
	...
	private void readValues(ByteBuffer buffer, int opcode) {
		if (opcode == 1) { interfaceModelId = buffer.getShort() & 0xFFFF; } else if (opcode == 2) {
			...
			modelOffset2 <<= 0;
		} else if (opcode == 11) { stackable = 1; } else if (opcode == 12) {
			value = buffer.getInt();
		} else if (opcode == 16) {
			membersOnly = true;
		} else if (opcode == 18) {
			...
			for (int index = 0; index < length; index++) { unknownArray1[index] = buffer.get(); }
		} else if (opcode == 65) {
			unnoted = true;
		} else if (opcode == 78) {
			...
			buffer.getShort();
		} else if (opcode == 96) { buffer.get(); } else if (opcode == 97) {
		...
		} else if (opcode == 110) { buffer.getShort(); } else if (opcode == 111) {
			buffer.getShort();
		} else if (opcode == 112) {
			buffer.getShort();
		} else if (opcode == 113) {
			buffer.get();
		} else if (opcode == 114) { buffer.get(); } else if (opcode == 115) {
			teamId = buffer.get();
		} else if (opcode == 121) {
			...
			for (int index = 0; index < length; index++) { unknownArray2[index] = buffer.getShort() & 0xFFFF; }
		} else if (opcode == 134) { buffer.get(); } else if (opcode == 139) {
	...
	public HashMap<Integer, Integer> getWearingRequirements() {
		...
		if (clientScriptData == null) { return skills; }
		int nextLevel = -1;
		int nextSkill = -1;
		for (int key : clientScriptData.keySet()) {
			...
			if (value instanceof String) { continue; }
			if (key >= 749 && key < 797) {
				if (key % 2 == 0) { nextLevel = (Integer) value; } else { nextSkill = (Integer) value; }
