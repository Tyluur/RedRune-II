...
public class PlayerRendering implements OutgoingPacketStructure {
		...
		switch (stage) {
			case REMOVE_PLAYER:
				if (p != null) {
					...
					} else if (p.getLocation().getZ() != p.getRenderData().getLastLocation().getZ()) {
			...
			case TELEPORTED:
				...
				if (deltaX <= 15 && deltaY <= 15) {
					...
					buffer.writeBits(1, 1);
					buffer.writeBits(30, (delta.getY() & 0x3fff) | ((delta.getX() & 0x3fff) << 14) | ((delta.getZ() & 0x3) << 28));
				}
				break;
			default:
				break;
	...
	private static void updateGlobalPlayer(Player player, Player p, PacketBuilder buffer, GlobalUpdateStage stage, PacketBuilder flagBased) {
		...
		switch (stage) {
			case ADD_PLAYER:
				if (p.getRenderData().getLastLocation() != null && p.getLocation().getZ() != p.getRenderData().getLastLocation().getZ()) {
					updateGlobalPlayer(player, p, buffer, GlobalUpdateStage.HEIGHT_UPDATED, flagBased);
				} else {
					updateGlobalPlayer(player, p, buffer, GlobalUpdateStage.TELEPORTED, flagBased);
				}
				...
				break;
			case HEIGHT_UPDATED:
				int z = p.getLocation().getZ() - p.getRenderData().getLastLocation().getZ();
				buffer.writeBits(2, z);
				break;
			case TELEPORTED:
				buffer.writeBits(18, (p.getLocation().getZ() << 16) | (((p.getLocation().getRegionX() >> 3) & 0xFF) << 8) | ((p.getLocation().getRegionY() >> 3) & 0xFF));
				break;
			default:
				break;
		}
