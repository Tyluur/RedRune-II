...
public class DefaultPathFinder implements PathFinder {
	...
	public PathState findPath(Entity mob, Location base, int srcX, int srcY, int dstX, int dstY, int z, int radius, boolean running, boolean ignoreLastStep, boolean moveNear) {
		...
		Location location = RegionManager.getLocation((base.getRegionX() - viewportDepth) << 3, (base.getRegionY() - viewportDepth) << 3, base.getZ());
		...
		if (!foundPath) {
			if (moveNear) {
				...
				int costValue = mob.getAttribute(AttributeKey.COST_VALUE, 1);
				if (fullCost > costValue) {
					state.routeFailed();
					if (costValue > 1) {
						mob.removeAttribute(AttributeKey.COST_VALUE);
					}
				}
				if (fullCost == 1000) {
					return state;
