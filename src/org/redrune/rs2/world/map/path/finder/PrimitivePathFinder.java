package org.redrune.rs2.world.map.path.finder;
...
import org.redrune.rs2.world.map.path.PathFinder;
import org.redrune.rs2.world.map.path.PathState;
...
public class PrimitivePathFinder implements PathFinder {
	...
	public PathState findPath(Entity mob, Location base, int srcX, int srcY, int dstX, int dstY, int z, int radius, boolean running, boolean ignoreLastStep, boolean moveNear, boolean nullOnFail) {
		...
		Location location = RegionManager.getLocation((base.getRegionX() - 6) << 3, (base.getRegionY() - 6) << 3, base.getZ());
		PathState state = new PathState();
		int curX = srcX;
		int curY = srcY;
		final int stepCount = running ? 6 : 3;
		for (int i = 0; i < stepCount; i++) {
			Location curLoc = RegionManager.getLocation(location.getX() + curX, location.getY() + curY, location.getZ());
