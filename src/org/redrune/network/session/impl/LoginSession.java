		...
		if (context instanceof LoginRequestEvent) {
			...
			if (request.getMajor() != Protocol.REVISION || request.getMinor() != Protocol.SUB_REVISION) {
				channel.writeAndFlush(
						new LoginResponseEvent(request.getLoginRequest(), ProtocolResponse.OUT_OF_DATE, null));
			} else {
				player = new Player(request.getUsername(), request.getPassword());
				player.addPlayerToWorld(request.getLoginRequest());
				channel.writeAndFlush(
						new LoginResponseEvent(request.getLoginRequest(), ProtocolResponse.SUCCESSFUL_LOGIN, player));
				channel.pipeline().addAfter("login.read", "game.write", new GameWriteEvent());
				channel.pipeline().replace("login.read", "game.read",
						ProtocolThrottle.setSession(channel, new GameReadEvent(request.getIsaacPair())));
				channel.attr(ChannelListener.CURRENT_SESSION)
						.set(new GameSession(player, channel, request.getIsaacPair()));
