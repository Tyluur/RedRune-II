...
public class ProtocolThrottle extends ByteToMessageDecoder {
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
		if (in.isReadable()) {
			...
			Protocol request = Optional.of(opcode == NetworkConstants.JS5_REQUEST ? Protocol.REQUEST_HANDSHAKE : opcode == NetworkConstants.LOGIN_REQUEST ? Protocol.REQUEST_LOGIN : null).get();
			switch (request) {
				case REQUEST_HANDSHAKE:
					...
					break;
				case REQUEST_LOGIN:
					ctx.channel().writeAndFlush(Unpooled.buffer(1).writeByte(ProtocolResponse.SUCCESSFUL_CONNECTION.getClientId()));
