	...
	public void sendWorldList(boolean full) {
		...
		if (full) {
			...
			for (WorldEntry world : WorldList.getWorlds().values()) {
				...
				packet.writeGJString(world.getCountryName());
			}
			packet.writeSmart(0);
			...
			for (int world = 1; world <= WorldList.getWorlds().size(); world++) {
				...
				packet.writeGJString(WorldList.getWorlds().get(world).getActivity()); // activity
				packet.writeGJString(WorldList.getWorlds().get(world).getIp()); // ip
			}
